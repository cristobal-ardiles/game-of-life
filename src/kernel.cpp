...
inline void check_neighbour(int i, int j, int rel_i, int rel_j, int n, int m, std::vector<int> state, int& counter){
...
void print_matrix(std::vector<int> mat, int n, int m){
  for (int i=0; i<n; i++){
    for (int j=0; j<m; j++){
      int index = i*m + j; 
      std::cout << mat[index];
